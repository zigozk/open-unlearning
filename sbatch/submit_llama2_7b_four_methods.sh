...
for trainer in ${METHODS}; do
  ...
  for name in LEARNING_RATE NUM_TRAIN_EPOCHS PER_DEVICE_TRAIN_BATCH_SIZE GRADIENT_ACCUMULATION_STEPS EXTRA_TRAIN_ARGS HF_DATASETS_CACHE HF_DATASETS_OFFLINE CUDA_MODULE COLLATOR; do
    if [[ -n "${!name:-}" ]]; then
